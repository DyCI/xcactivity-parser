...
def test_usage_message_on_no_params():
	sp = subprocess.Popen([script_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	out, err = sp.communicate()
	assert "Usage:" in string_from_std(err)
...
def test_usage_message_on_only_arch_parameter():
	sp = subprocess.Popen([script_path, "-a", "i386"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	out, err = sp.communicate()
	assert "Usage:" in string_from_std(err)	
...
def test_searching_message_on_all_parameters():
	sp = subprocess.Popen([script_path, "-f", "/", "-x", "/", "-a", "i386"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	out, err = sp.communicate()
	assert not "Usage:" in string_from_std(err)

def test_full_path_for_file():
	sp = subprocess.Popen([script_path, "-f", "asd", "-x", "/"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	out, err = sp.communicate()
	assert "full path" in string_from_std(err)

def test_directory_existance():
	sp = subprocess.Popen([script_path, "-f", "/", "-x", "asfsd"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	out, err = sp.communicate()
	assert "directory" in string_from_std(err)
	assert "exist" in string_from_std(err)
...
def test_arguments_on_existed_file():
	...
	sp = subprocess.Popen([script_path, "-f", existingfile, "-x", fixtures_path, "-a", "i386"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	out, err = sp.communicate()
	assert not "Usage:" in string_from_std(err)
	# print out
	# print err
	assert compilationString == string_from_std(out).strip()
...
def test_arguments_on_existed_file():
	...
	sp = subprocess.Popen([script_path, "-f", existingfile, "-x", fixtures_path, "-a", "x86_64"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	out, err = sp.communicate()
